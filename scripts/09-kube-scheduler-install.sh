...
do
{
  ...
  ssh root@${master_ip} "systemctl daemon-reload; systemctl enable kube-scheduler.service --now; sleep 10s;"
  ssh root@${master_ip} "systemctl status kube-scheduler.service | grep 'Active:';"
